...
class MLService:
    ...
    def _generate_recommendation(self, results: Dict) -> Dict:
        """Return an honest placeholder recommendation.

        Carbon-credit eligibility and credit volume cannot be derived from model
        output alone — they require field verification and certification. This
        service does not fabricate them.
        """
        return {
            "note": "Automated recommendation not available; requires human verification"
        }
    ...
    async def get_explanation(self, explanation_id: str) -> Optional[Dict[str, Any]]:
        ...
        The fabricated-explanation store has been removed, so nothing is ever
        persisted here. This returns None (surfaced by the legacy endpoint as a
        404) rather than inventing an explanation.
        """
        return None
