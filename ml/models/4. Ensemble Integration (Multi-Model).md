...
The ensemble model is a multi-model system that combines three specialized deep learning models for carbon credit verification.
...
2. **Change Detection Siamese U-Net (F1=0.6006)**
   ...
   - **Strength:** High recall (97.06%) for detecting forest changes
...
- **Reuses existing work:** Builds on already-trained, tested models
...
- **Performance-weighted:** Design target F1 > 0.6 (not benchmarked)
...
| **Change Detection Siamese** | 0.6006 | 0.4349 | 0.9706 | High-recall change detection | 50.4% |
...
**Expected Results (projections, not measured):**
- **F1 Score:** > 0.60 (design target; the ensemble has not been evaluated on a held-out test set — note the best individual model, change detection, is already at F1≈0.60)
- **Precision:** ~0.50 (projected)
- **Recall:** ~0.75 (projected)
- **Carbon Calculation:** deterministic area-based formula (no independently validated accuracy figure)

### 4.3 Evaluation Status
The only metrics recorded from actual evaluation are the individual model F1 scores
(Forest Cover F1 = 0.49, Change Detection F1 = 0.60; see `ml/evaluation_results/`).
The ensemble itself has not been benchmarked on a held-out test set, and there are no
validated "overall performance", "carbon accuracy", or "production readiness" scores.
The composite grades previously shown here (e.g. "0.720/1.000 B+", per-component
reliability scores, "0.991 carbon accuracy", "0.95 production readiness") were not backed
by measurements and have been removed.
...
The carbon figure is a deterministic area-based calculation (forest pixels × pixel area × carbon-per-hectare).
Because it is a fixed formula rather than a learned model, it has no independently validated accuracy figure.
The previously reported "99.1% accuracy", "0.998 correlation", and "96.7% within 5% error" were not backed by a
real validation study and have been removed.
...
- **Carbon Calculations:** deterministic area-based formula (no independently validated accuracy figure)  
...
This is a research/prototype pipeline, not a production-validated system. Model accuracy is limited
(forest-cover F1 ≈ 0.49, change-detection F1 ≈ 0.60), the ensemble is unbenchmarked, and there is no
validated carbon-accuracy figure. The previously shown "95/100 Production Ready" score and its
component sub-scores were not backed by measurements and have been removed.
...
The ensemble model combines three specialized deep learning models into a single pipeline. It is a research prototype, not a production-validated system.

### 13.1 Technical Achievements
- **Performance:** Expected F1 > 0.6 (design target; the ensemble has not been benchmarked)
- **Testing:** Component-level tests exist; no production-readiness score is claimed
- **Carbon Calculation:** deterministic area-based formula (no independently validated accuracy figure)
- **System Integration:** Multi-model coordination across the three models
- **Extensible Architecture:** Framework supports adding further models

### 13.2 Business Impact
- **Immediate Value:** Leverages existing trained models
- **Cost Efficiency:** Reuses trained components rather than training from scratch
- **Foundation:** Provides a working starting point for carbon credit verification
- **Combination:** Aims to improve on individual models by combining them (pending benchmarking)
- **Future Proof:** Extensible framework for continuous improvement

### 13.3 Intended Environmental Application
- **Forest Monitoring:** Detection of forest changes
- **Carbon Verification:** Quantification of carbon sequestration (pending validation)
...
**Status**: Research prototype — not production-ready. Model accuracy is limited (forest-cover F1 ≈ 0.49, change-detection F1 ≈ 0.60) and the ensemble has not been benchmarked.

The ensemble combines the three trained models into one pipeline. Its expected F1 (> 0.6) is a design target that has not yet been benchmarked, and the carbon figure is a deterministic area-based estimate with no independently validated accuracy. Further evaluation is required before real-world carbon credit verification use.

**Integration points:** The pipeline exposes a CLI and Python API that web interfaces and (future) blockchain or trading integrations can build on, once model performance has been validated. 