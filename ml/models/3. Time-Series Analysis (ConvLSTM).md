...
- **Convergence Speed:** Fast (early stopped at 5 epochs)
...
- **Ensemble Enhancement:** Intended to contribute to the ensemble's expected F1 > 0.6 (design target, not benchmarked)
...
**Status**: Trained and integrated as an ensemble component (research prototype). Note: standalone F1 = 0 due to the class imbalance described above; the ensemble has not been benchmarked on a held-out test set.

The ConvLSTM model is integrated as the temporal analysis component of the carbon credit verification pipeline, intended to add temporal context to the spatial models. Its contribution to overall ensemble performance has not yet been measured. 